...
public class DefaultCatalogProblemBuilder {
    ...
    public static void maybeThrowError(InternalProblems problemService, String error, Collection<Problem> problems) {
        if (!problems.isEmpty()) {
            throw throwErrorWithNewProblemsApi(problemService, error, problems);
        }
    }

    public static RuntimeException throwErrorWithNewProblemsApi(InternalProblems problemService, String error, Collection<Problem> problems) {
